...
class PublisherNode(Node):
    ...
    def track_atag(self):
            ...
            if len(tags) == 0:
                ...
                if not st_argv_for_arm=="50 0 0 0": #try to lock atag
                    ...
                    if not self.flg_z_m_40: # ok for locking atag with high z
                        self.publisher_.publish(String(data="20 0 0 -30")) # z -40 # ```````````````````````````````````
                ...
                if st_argv_for_arm=="50 0 0 0" and self.flg_z_m_40_done :
                    # ok for locking atag 
    ...
    def move_cube(self):
        ...
        st1=f"80 -80 0 0" # move wrist to view-center dx dy dz
        self.get_logger().info(f'{st1}')
        self.publisher_.publish(String(data=st1))      
        #time.sleep(1.0)   


        st1=f"70 0 0 0" # claw parallet +y-y 
        self.get_logger().info(f'{st1}')
        self.publisher_.publish(String(data=st1))      
        #time.sleep(1.0)

        st1=f"20 0 0 -20" # claw z -20
        self.get_logger().info(f'{st1}')
        self.publisher_.publish(String(data=st1)) 
        
        st1=f"14 0 0 0" # catch bottle 
        self.get_logger().info(f'{st1}')
        self.publisher_.publish(String(data=st1)) 
        
        
        st1=f"20 10 0 10" # claw z +10
        self.get_logger().info(f'{st1}')
        self.publisher_.publish(String(data=st1))                 
        ...
        st1=f"0 0 0 0" # wrist home
        self.get_logger().info(f'{st1}')
        self.publisher_.publish(String(data=st1)) 
        
        st1=f"5 0 0 0" # servo#5 home
        self.get_logger().info(f'{st1}')
        self.publisher_.publish(String(data=st1))        
        
        st1=f"60 -120 0 30" # rotate bottle
        self.get_logger().info(f'{st1}')
        self.publisher_.publish(String(data=st1))
 
        st1=f"60 120 120 -30" # to desk
        self.get_logger().info(f'{st1}')
        self.publisher_.publish(String(data=st1))

        st1=f"11 0 0 0" # release bottle
        self.get_logger().info(f'{st1}')
        self.publisher_.publish(String(data=st1))

        st1=f"30 -60 0 0" # radius direction back
        self.get_logger().info(f'{st1}')
        self.publisher_.publish(String(data=st1))
